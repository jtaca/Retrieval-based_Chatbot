...
def compare_same_questions(processed_questions, processed_answers):
    if(os.path.isfile('./compare_same_questions.txt')):
        ...
        test_for_same_answers = load_dict_from_file(
            'test_for_same_answers.txt')
    else:
        samex = []
        samei = []
        same_question_id = {}
                        ...
                        if(x != i):
                            try:
                                ...
                                if(jac > 0.9 and i not in samex and i not in samei):
                                    ...
                                    jac_answer = get_dice_sim(
                                        processed_answers[str(i)], processed_answers[str(x)])
                                    if (jac_answer < 0.8):
...
def weighted_score(id_questions, processed_answers, user_input):
    ...
    for id in id_questions:
        ...
        for question in processed:
            ...
            # + textdistance.ratcliff_obershelp(user_input, question) #use for 2% more add this to report
            jac_question = get_dice_sim(user_input, question)
    ...
    for ida in id_questions:
        score[ida] = score_answer[ida]*0.1 + \
            score_question[ida]*0.7 + same_question_id[ida] * 0.2
...
def main():
    ...
    # change to processed_questions if not in test
    compare_same_questions(id_questions, processed_answers)
    Test_accuracy()
