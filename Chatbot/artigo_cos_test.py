            ...
            for y in processed_questions[x]:
                for i in processed_questions:
                    for j in processed_questions[i]:
                        if(x != i):
                            try:
                                jac= get_cos_dist(y,j)
                                if( jac> 0.9 and i not in samex and i not in samei ):
                                    test_for_same_answers[i] = x
                                    jac_answer = get_cos_dist(processed_answers[str(i)],processed_answers[str(x)])
                                    if (jac_answer<0.8):
...
def weighted_score(id_questions,processed_answers, user_input):
    ...
    for id in id_questions:
        ...
        score_answer[id] = get_cos_dist(user_input,processed_answers[id])
        max_question = 0
        for question in processed:
            
            jac_question = get_cos_dist(user_input,question)#+ textdistance.ratcliff_obershelp(user_input, question) #use for 2% more add this to report
